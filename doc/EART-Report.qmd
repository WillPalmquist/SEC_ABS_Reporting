...
To access, download, and aggregate the monthly asset data files, see `01-SEC-Scraping.R` in the associated github repo found [here](https://github.com/WillPalmquist/SEC_ABS_Reporting). All associated scripts, quarto documents, and data files can also be found in the repository.
...
At the highest level, I wanted to understand the total loan amount and how this value is paid down over time. The figure below shows a simple time series of the overall loan amount and the total balance remaining aggregated at the monthly level. In terms of time to maturity, the ABS is still early in its overall life. After the first few months in which individual loans accumulate, there is a stable decline in overall principal amount as borrowers make payments against their loan balance.
